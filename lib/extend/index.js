...
_extend.prototype.select = function(model, opt) {
  ...
  this._skip =  this._skip == undefined ? this.page(model.page || null ,null,true) : this._skip;
  this._limit = this._limit == undefined ?  this.limit(model.limit || null, null, true) : this._limit;
  this._rtype = this._rtype == undefined ?  this.type(model._rtype || null, null, true) : this._rtype;
...
_extend.prototype.where = function(criteria, opt) {
  ...
  this._select = _.isEmpty(criteria.select) ? query.select().native() : query.select().custom(criteria.select);
  this._skip =  this._skip == undefined ? this.page(criteria.page || null ,null,true) : this._skip;
  this._limit = this._limit == undefined ?  this.limit(criteria.limit || null, null, true) : this._limit;
  this._rtype = this._rtype == undefined ?  this.type(criteria._rtype || null, null, true) : this._rtype;
...
_extend.prototype.page = function(_page, opt, returnVal) {
  var callback = opt;
  this._skip = query.page(_page);
  if(typeof opt != 'function' && returnVal != true) return this;
  if(returnVal == true) return this._skip;
  this.exec(callback);
};

_extend.prototype.limit = function(_limit, opt, returnVal) { 
  var callback = opt;
  this._limit = query.limit(_limit);
  if(typeof opt != 'function' && returnVal != true) return this;
  if(returnVal == true) return this._limit;
  this.exec(callback);
};
...
_extend.prototype.type = function(_type, opt, returnVal) {
  var callback = opt;
  this._rtype = query.type(_type);
  if(typeof opt != 'function' && returnVal != true) return this;
  if(returnVal == true) return this._rtype;
  this.exec(callback);
};
