...
function _process(obj, creds, options){
 ...
 // try{
   ...
   mbody = ext(obj, mbody);
   creds._request({form:mbody, url:creds.host+base.input, timeout:creds.configs.timeout, method:"POST"}, function(err,res, data){  console.log(err ? err: data ? data : 'empty');  });
 // } catch(e) { throw e; }
}
